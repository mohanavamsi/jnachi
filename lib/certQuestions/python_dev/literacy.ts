...
export const PYTHON_DEV_LITERACY_QUESTIONS: CertQuestion[] = [
  {
    ...
    options: [
      { id: "a", label: "Using the `match / case` syntax with pattern guards and sequence/mapping destructuring." },
      { id: "b", label: "Chaining 25 nested `if/elif/else` statements using string equality." },
  ...
  },
  {
    ...
    options: [
      { id: "a", label: "Generators evaluate items lazily on-demand (O(1) memory consumption) without loading the entire dataset into RAM simultaneously." },
      { id: "b", label: "Generators permanently delete logs from disk as they are read." },
      { id: "c", label: "Generators bypass the Python Global Interpreter Lock (GIL) automatically." },
      { id: "d", label: "Generators compress string data into binary zip files." }
    ],
    correctOptionId: "a"
  },
  {
    ...
    options: [
      { id: "a", label: "`def process(val: str | int | None) -> None:`" },
      { id: "b", label: "`def process(val: any(str, int, null)):`" },
      { id: "c", label: "`def process(val: [str, int, void]):`" },
  ...
  },
  {
    ...
    options: [
      { id: "a", label: "To encapsulate resource setup and deterministic cleanup (e.g. file closing, db connections, lock releases) even if unhandled exceptions occur." },
      { id: "b", label: "To run Python scripts in parallel across multiple physical server clusters." },
  ...
  },
  {
    ...
    options: [
      { id: "a", label: "Memoizes function return values based on positional and keyword arguments, returning cached results for repeat calls with O(1) lookup." },
      { id: "b", label: "Converts synchronous functions into multithreaded daemon processes." },
      { id: "c", label: "Writes function outputs directly to a local SQLite database." },
  ...
  },
  {
    ...
    options: [
      { id: "a", label: "The default list is instantiated once at function definition time, so mutations persist across all subsequent function calls across the application." },
      { id: "b", label: "Python throws a `SyntaxError` at compile time." },
      { id: "c", label: "The list is automatically converted into an immutable tuple." },
      { id: "d", label: "It causes the Python interpreter to freeze immediately." }
    ],
    correctOptionId: "a"
  },
  {
    ...
    options: [
      { id: "a", label: "Allows catching and handling multiple independent exceptions simultaneously that occurred in concurrent tasks (e.g. TaskGroups)." },
      { id: "b", label: "Suppresses all exceptions silently without logging." },
