...
export const PYTHON_DEV_PRIVACY_QUESTIONS: CertQuestion[] = [
  {
    ...
    options: [
      { id: "a", label: "`pickle` can deserialize arbitrary Python objects, including constructors that invoke `os.system()` or `subprocess`; use `json` or `msgpack` for untrusted payloads." },
      { id: "b", label: "`pickle` only works on 32-bit operating systems." },
  ...
  },
  {
    ...
    options: [
      { id: "a", label: "Use parameterized queries / prepared statements (e.g. `cursor.execute('SELECT * FROM users WHERE id = %s', (user_id,))`) or ORM query builders (SQLAlchemy/SQLModel)." },
      { id: "b", label: "Use raw Python string formatting: `f\"SELECT * FROM users WHERE id = {user_input}\"`." },
      { id: "c", label: "Concatenate strings with `+` and disable SQL server error logs." },
  ...
  },
  {
    ...
    options: [
      { id: "a", label: "`secrets` (e.g. `secrets.token_urlsafe(32)`)." },
      { id: "b", label: "`random.random()` (Mersenne Twister, which is pseudo-random and predictable)." },
      { id: "c", label: "`time.time()` converted to an integer." },
      { id: "d", label: "`math.pi` sliced to 10 decimal digits." }
    ],
    correctOptionId: "a"
  },
  {
    ...
    options: [
      { id: "a", label: "`bcrypt`, `argon2-cffi`, or `passlib` with a unique cryptographic salt and high work factor." },
      { id: "b", label: "Plain MD5 or SHA-1 hashes without salts." },
  ...
  },
  {
    ...
    options: [
      { id: "a", label: "Custom logging Filters or formatters that redact passwords, credit card numbers, and authorization headers before outputting logs." },
      { id: "b", label: "Logging all incoming HTTP request bodies and headers at `DEBUG` level to public stdout." },
      { id: "c", label: "Disabling error logs so security auditors cannot find flaws." },
  ...
  },
  {
    ...
    options: [
      { id: "a", label: "Explicitly allow only trusted front-end domains in `allow_origins=['https://app.yourdomain.com']` and disallow wildcards (`*`) with credentials." },
      { id: "b", label: "Set `allow_origins=['*']` and `allow_credentials=True` globally." },
      { id: "c", label: "Disable CORS middleware completely." },
      { id: "d", label: "Allow only localhost URLs in production environments." }
    ],
    correctOptionId: "a"
  },
  {
    ...
    options: [
      { id: "a", label: "Attacker passing `../../etc/passwd` to file paths; prevented by resolving absolute paths with `path.resolve()` and asserting `resolved_path.is_relative_to(base_directory)`." },
      { id: "b", label: "A hardware hard drive failure." },
  ...
  },
  {
    ...
    options: [
      { id: "a", label: "Secrets become permanently recorded in git commit history and can be discovered by unauthorized contributors, scrapers, or compromised repos." },
      { id: "b", label: "Git refuses to commit files starting with a dot." },
