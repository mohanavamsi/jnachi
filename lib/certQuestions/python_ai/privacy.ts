...
export const PYTHON_AI_PRIVACY_QUESTIONS: CertQuestion[] = [
  {
    ...
    options: [
      { id: "a", label: "Use environment variables loaded via `os.environ.get()` or `pydantic-settings` / `.env` files that are strictly excluded in `.gitignore`." },
      { id: "b", label: "Hardcode the raw plaintext API keys directly into public GitHub repository files." },
  ...
  },
  {
    ...
    options: [
      { id: "a", label: "Use redacting pipelines (e.g. Microsoft Presidio or regex anonymizers) to replace names, SSNs, and credit cards with synthetic placeholders like `[NAME_1]`." },
      { id: "b", label: "Append a prompt saying 'Please don't look at the customer credit cards in this prompt'." },
      { id: "c", label: "Log the full raw unencrypted customer database dump to public S3 buckets." },
  ...
  },
  {
    ...
    options: [
      { id: "a", label: "Untrusted user inputs manipulating LLM instructions; defend against it using strict system/user message boundaries, XML tags (`<user_data>`), guardrails (NeMo Guardrails), and schema enforcement." },
      { id: "b", label: "A hardware failure in GPU tensor cores caused by overclocking." },
      { id: "c", label: "Injecting CSS stylesheets into Python backend memory." },
      { id: "d", label: "A standard Python syntax error caused by missing colons." }
    ],
    correctOptionId: "a"
  },
  {
    ...
    options: [
      { id: "a", label: "Mask or redact all sensitive keys, authorization tokens, passwords, and sensitive customer data before writing to centralized log stores." },
      { id: "b", label: "Log all plaintext master passwords to standard error." },
  ...
  },
  {
    ...
    options: [
      { id: "a", label: "Filtering vector search queries by user tenancy (`tenant_id == current_user.tenant_id`) to ensure users can never retrieve documents outside their authorized organizational domain." },
      { id: "b", label: "Translating document texts into multiple spoken languages." },
      { id: "c", label: "Compressing images stored alongside the text." },
  ...
  },
  {
    ...
    options: [
      { id: "a", label: "All inference data, embeddings, and context stay 100% within the organization's private VPC/on-premises network without third-party egress." },
      { id: "b", label: "Local models are guaranteed to never produce hallucinations." },
      { id: "c", label: "Hardware cooling fans are no longer required." },
      { id: "d", label: "Python code will never encounter runtime exceptions." }
    ],
    correctOptionId: "a"
  },
  {
  ...
  },
  {
    ...
    options: [
      { id: "a", label: "Isolate memory instances per authenticated user session and purge transient context when the session terminates or times out." },
      { id: "b", label: "Store all user messages in a single shared global Python list." },
      { id: "c", label: "Broadcast all incoming user prompts to all connected WebSockets." },
